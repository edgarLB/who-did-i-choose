...
export default function Lobby() {
    ...
    const inviteCode = params.gameId;
    ...
    // Fetch game, players, & subscribe to realtime changes
    useEffect(() => {
        if (!inviteCode) return;
        let channel: ReturnType<typeof supabase.channel> | undefined;

        async function fetchGameAndPlayers() {
            // Resolve the game ID via the 6‑character invite code
            const { data: game, error: gameError } = await supabase
                .from("games")
                .select("id")
                .eq("game_code", inviteCode)
                .single();
